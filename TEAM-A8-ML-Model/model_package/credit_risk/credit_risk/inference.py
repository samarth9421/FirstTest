...
def predict_single(input_data, applicationID):
    ...
    try: 
        ...
        assert isinstance(data, pd.core.frame.DataFrame), "data should be a pandas dataframe"
        assert len(data_row) == 1, "There should be a one-one mapping between application id and data row"
...
def individual_interpretation(input_data, file_path, applicationID=None):
    ...
    try: 
        ...
        assert applicationID != None or data.shape[0] == 1, "Application ID or data_row must be given"
        ...
        assert len(data_row) == 1, "A single data row should be given"
